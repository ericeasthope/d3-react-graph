...
var runForceSimulation = function runForceSimulation(nodes, links) {
  ...
  simulation.alphaDecay(1e-3).velocityDecay(4e-1).force('charge', d3.forceManyBody().strength(-150)).force('x', d3.forceX()).force('y', d3.forceY()).force('link', d3.forceLink(links).id(function (d) {
    return d.id;
