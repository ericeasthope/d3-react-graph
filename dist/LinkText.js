...
var LinkText = function LinkText(_ref) {
  var children = _ref.children,
      links = _ref.links;
  var g = d3.select('.links').selectAll('g').data(links).join(function (enter) {
  ...
  children.map(function (c) {
    var text = g.append('text').text(c.text).attr('text-anchor', 'middle').style('font-size', c.fontSize).attr('fill', c.fill).attr('dy', c.dy).attr('alignment-baseline', 'middle');
    text.on('mouseover', function () {
      c.onMouseOver ? c.onMouseOver(d3.select(this)) : null;
    }).on('mouseout', function () {
      c.onMouseOut ? c.onMouseOut(d3.select(this)) : null;
    });
    c.onClick ? text.on('click', function (d) {
  ...
  var tick = function tick() {
    ...
    */
    .attr('transform', function (d) {
      return 'translate(' + (d.source.x + d.target.x) / 2 + ',' + (d.source.y + d.target.y) / 2 + ')' + 'rotate(' + 180 / Math.PI * Math.atan2(d.target.y - d.source.y, d.target.x - d.source.x) + ')';
    });
  };
